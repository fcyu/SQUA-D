...
generateUpspTable <- function(i, allUpsp, psmTableFinal) {
  ...
  upspTable <- data.frame(
    ...
    IDs_light = sum(psmTable$label_type == "L"),
    IDs_heavy = sum(psmTable$label_type == "H"),
    light_IDs_from_control_1 = sum(str_detect(psmTable$experiment_type, "f1") & (psmTable$label_type == "L")),
    heavy_IDs_from_treatment_1 = sum(str_detect(psmTable$experiment_type, "f1") & (psmTable$label_type == "H")),
    total_IDs_f1 = sum(str_detect(psmTable$experiment_type, "f1")),
    light_IDs_from_control_2 = sum(str_detect(psmTable$experiment_type, "f2") & (psmTable$label_type == "L")),
    heavy_IDs_from_treatment_2 = sum(str_detect(psmTable$experiment_type, "f2") & (psmTable$label_type == "H")),
    total_IDs_f2 = sum(str_detect(psmTable$experiment_type, "f2")),
    light_IDs_from_control_3 = sum(str_detect(psmTable$experiment_type, "f3") & (psmTable$label_type == "L")),
    heavy_IDs_from_treatment_3 = sum(str_detect(psmTable$experiment_type, "f3") & (psmTable$label_type == "H")),
    total_IDs_f3 = sum(str_detect(psmTable$experiment_type, "f3")),
    forward_replication = (sum(str_detect(psmTable$experiment_type, "f1")) > 0) + (sum(str_detect(psmTable$experiment_type, "f2")) > 0) + (sum(str_detect(psmTable$experiment_type, "f3")) > 0),
    light_IDs_from_treatment_1 = sum(str_detect(psmTable$experiment_type, "r1") & (psmTable$label_type == "L")),
    heavy_IDs_from_control_1 = sum(str_detect(psmTable$experiment_type, "r1") & (psmTable$label_type == "H")),
    total_IDs_r1 = sum(str_detect(psmTable$experiment_type, "r1")),
    light_IDs_from_treatment_2 = sum(str_detect(psmTable$experiment_type, "r2") & (psmTable$label_type == "L")),
    heavy_IDs_from_control_2 = sum(str_detect(psmTable$experiment_type, "r2") & (psmTable$label_type == "H")),
    total_IDs_2 = sum(str_detect(psmTable$experiment_type, "r2")),
    light_IDs_from_treatment_3 = sum(str_detect(psmTable$experiment_type, "r3") & (psmTable$label_type == "L")),
    heavy_IDs_from_control_3 = sum(str_detect(psmTable$experiment_type, "r3") & (psmTable$label_type == "H")),
    total_IDs_f3 = sum(str_detect(psmTable$experiment_type, "r3")),
    reciprocal_replication = (sum(str_detect(psmTable$experiment_type, "r1")) > 0) + (sum(str_detect(psmTable$experiment_type, "r2")) > 0) + (sum(str_detect(psmTable$experiment_type, "r3")) > 0),
    all_replication = (sum(str_detect(psmTable$experiment_type, "f1")) > 0) + (sum(str_detect(psmTable$experiment_type, "f2")) > 0) + (sum(str_detect(psmTable$experiment_type, "f3")) > 0) + (sum(str_detect(psmTable$experiment_type, "r1")) > 0) + (sum(str_detect(psmTable$experiment_type, "r2")) > 0) + (sum(str_detect(psmTable$experiment_type, "r3")) > 0),
    all_IDs = nrow(psmTable),
    light_in_forward_control = sum(str_detect(psmTable$experiment_type, "f") & (psmTable$label_type == "L")),
    heavy_in_reciprocal_control = sum(str_detect(psmTable$experiment_type, "r") & (psmTable$label_type == "H")),
    total_control = sum(str_detect(psmTable$experiment_type, "f") & (psmTable$label_type == "L")) + sum(str_detect(psmTable$experiment_type, "r") & (psmTable$label_type == "H")),
    light_in_reciprocal_treatment = sum(str_detect(psmTable$experiment_type, "r") & (psmTable$label_type == "L")),
    heavy_in_forward_treatment = sum(str_detect(psmTable$experiment_type, "f") & (psmTable$label_type == "H")),
    total_treatment = sum(str_detect(psmTable$experiment_type, "r") & (psmTable$label_type == "L")) + sum(str_detect(psmTable$experiment_type, "f") & (psmTable$label_type == "H")),
